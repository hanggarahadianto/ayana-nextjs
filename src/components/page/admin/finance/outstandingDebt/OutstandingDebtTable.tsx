...
import { Group, ScrollArea, Table, Badge, Text } from "@mantine/core";
...
export default function OutstandingDebtTable({ data, startIndex = 1, companyId, status }: OutstandingDebtTableProps) {
          ...
          >
            <Table.Thead>
              <Table.Tr>
                ...
                {status !== "done" && <Table.Th style={{ textAlign: "center", width: 60, minWidth: 60 }}>Action</Table.Th>}
              ...
              {data.map((OutstandingDebt: IDebtSummaryItem, index: number) => {
                ...
                return (
                  <Table.Tr key={OutstandingDebt.id} style={{ cursor: "pointer" }}>
                    ...
                    {OutstandingDebt.status !== "done" && (
                      <Table.Td>
                        <IoIosSend color="green" size={"22px"} onClick={() => handleSendClick(OutstandingDebt)} />
                      </Table.Td>
                    )}
