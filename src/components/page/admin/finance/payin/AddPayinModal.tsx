...
const AddPayinModal = ({ refetchPayloadData, companyCode, companyId }: AddPayinModalProps) => {
  const [opened, { open, close }] = useDisclosure(false);

  const { mutate: postData, isPending: isLoadingSubmitAccountData } = useSubmitPayoutForm(refetchPayloadData, close);
                ...
                <SimpleGrid p={20}>
                  <Stack gap={20}>
                    <SelectFinanceAccount
                      companyId={companyId ?? ""}
                      onSelect={({ id, code, type }) => {
                        setFieldValue("account_id", id); // simpan ID
                        setFieldValue("account_code", code); // simpan code
                        setFieldValue("account_type", type); // simpan type
                      }}
                    />
                  ...
                  <Group justify="flex-end" mt="md">
                    ...
                    <Button type="submit" loading={isLoadingSubmitAccountData}>
                      Tambah Payout
                    </Button>
                  </Group>
                </SimpleGrid>
