...
// import { Modal, TextInput, Button, Group, Select, Card, Text, Stack, InputWrapper, SimpleGrid, Flex, Grid, Divider } from "@mantine/core";
...
// import { getInitialValuesCashFlow, initialValuesCashFlowUpdate } from "@/utils/initialValues/initialValuesCashFlow";
...
const EditCashFlowReportModal = ({
  ...
  //   return cashFlowData.filter((item) => item[0].id !== initialData[0].id).flatMap((res) => res.data.map((i) => i.week_number));
  // }, [cashFlowData, initialData[0].id]);

  // const availableWeeks = useMemo(() => {
  //   return allWeeks.filter((week) => !selectedWeeks.includes(week)).map((week) => ({ value: week, label: week }));
  // }, [selectedWeeks]);

  const handleSubmit = async (values: ICashFlowResponse["data"][0]) => {
    // try {
    //   updateDataCashFlow(
    //     { id: cashFlowData.id, values },
    //     {
    //       onSuccess: () => {
    //         const goodsWithCashFlowId = (values.good || []).map((good) => ({
    //           ...good,
    //           cash_flow_id:cashFlowData[0].id,
    //         }));
    //         updateDataGoods(goodsWithCashFlowId, {
    //           onError: () => console.error("Gagal menyimpan data barang."),
    //         });
    //         showNotification({
    //           title: "Berhasil",
...
//    <Formik
//      initialValues={getInitialValuesCashFlow(cashFlowData[0])}
//      // initialValues={getInitialValuesCashFlow(cashFlowData[0])}
//      validationSchema={validationSchemaCashFlowCreate}
//      validateOnBlur={false}
//      enableReinitialize
...
//                     data={availableWeeks.concat({
//                       value: values.week_number,
//                       label: values.week_number,
//                     })}
//                     error={touched.week_number && errors.week_number ? errors.week_number : undefined}
//                     required
//                   /> */}
...
//                  type="submit"
//                  color="blue"
//                  // disabled={isLoading || isLoadingPostDataGoods}
//                  // loading={isLoadingEdit || isLoadingPostDataGoods}
//                >
//                  Simpan Perubahan
//                </Button>
//              </Group>
